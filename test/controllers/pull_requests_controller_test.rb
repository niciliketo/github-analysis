...
class PullRequestsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create pull_request' do
    assert_difference('PullRequest.count') do
      post repo_pull_requests_url(@repo),
           params: { pull_request: { creator: @pull_request.creator, milestone: @pull_request.milestone,
                                     number: @pull_request.number, pr_created_at: @pull_request.pr_created_at, pr_merged_at: @pull_request.pr_merged_at } }
    end
  ...
  test 'should update pull_request' do
    patch repo_pull_request_url(@repo, @pull_request),
          params: { pull_request: { creator: @pull_request.creator, milestone: @pull_request.milestone,
                                    number: @pull_request.number, pr_created_at: @pull_request.pr_created_at, pr_merged_at: @pull_request.pr_merged_at } }
